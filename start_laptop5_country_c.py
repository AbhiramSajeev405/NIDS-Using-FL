...
def start_country_server():
    ...
    country_ip = config['network']['countries']['country_C']['ip']
    country_port = config['network']['countries']['country_C']['server_port']
...
def start_clients():
    """Start Client_07, Client_08, Client_09."""
    config = get_network_config()

    country_ip = config['network']['countries']['country_C']['ip']
    country_port = config['network']['countries']['country_C']['server_port']

    clients = ["Client_07", "Client_08", "Client_09"]
    processes = []
...
def main():
    ...
    print("="*70)
    print("LAPTOP 5: COUNTRY C")
    print("="*70)
    print(f"Your IP: {config['network']['countries']['country_C']['ip']}")
    print("Clients: Client_07, Client_08, Client_09")
    print("="*70)
    print("\nIMPORTANT:")
    print("1. Ensure Global Server (Laptop 2) is already running")
    print("2. Your IP is set to: 192.168.1.103")
    print("3. Firewall allows ports 9003, 8081")
    print("="*70)

    input("\nPress ENTER when ready to start Country A...")

    # Start country server
    print("\n[Step 1/2] Starting Country C Server...")
