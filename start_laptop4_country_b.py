...
def start_country_server():
    ...
    country_ip = config['network']['countries']['country_B']['ip']
    country_port = config['network']['countries']['country_B']['server_port']
...
def start_clients():
    """Start Client_04, Client_05, Client_06."""
    config = get_network_config()

    country_ip = config['network']['countries']['country_B']['ip']
    country_port = config['network']['countries']['country_B']['server_port']

    clients = ["Client_04", "Client_05", "Client_06"]
    processes = []
...
def main():
    ...
    print("="*70)
    print("LAPTOP 4: COUNTRY B")
    print("="*70)
    print(f"Your IP: {config['network']['countries']['country_B']['ip']}")
    print("Clients: Client_04, Client_05, Client_06")
    print("="*70)
    print("\nIMPORTANT:")
    print("1. Ensure Global Server (Laptop 2) is already running")
    print("2. Your IP is set to: 192.168.1.102")
    print("3. Firewall allows ports 9002, 8081")
    print("="*70)

    input("\nPress ENTER when ready to start Country A...")

    # Start country server
    print("\n[Step 1/2] Starting Country B Server...")
